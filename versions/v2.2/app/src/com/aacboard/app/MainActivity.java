 ...
 *  - unlock() -> stopLockTask(), then immediately brings this activity back to
 *               the front (see bringSelfToFront) because Android's own reaction
 *               to leaving a pinned task is to drop the user at the launcher.
...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // The segmentation model and the ONNX WASM runtime are assets loaded
        // by XHR from the file:// page. Without this they are cross-origin to
        // their own page and the read is refused, so background removal would
        // silently fall back to the colour-only pass on the device while
        // working perfectly in a desktop browser.
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            try {
                if (locked && !wasLocked) {
                    ...
                    // Leaving a pinned task sends the user to the launcher --
                    // Android treats "stop pinning" as "leave this task", so
                    // tapping Unlock looked like the app quitting. The activity
                    // is still alive; put it back in front so Unlock reads as
                    // unlock. Delayed a beat: reordering to front while the
                    // system is still tearing pinning down is ignored.
                    web.postDelayed(this::bringSelfToFront, 350);
                }
            } catch (Exception e) {
    ...
    /** Re-fronts this (still-running) activity after pinning is released. */
    private void bringSelfToFront() {
        try {
            ...
            android.util.Log.d("AAC_APP", "Re-fronted after unpin");
        } catch (Exception e) {
            android.util.Log.w("AAC_APP", "re-front after unpin failed: " + e);
        }
    }

    /**
     * The user can also leave pinning with Android's own back+overview gesture,
     * which never goes through our bridge. Ask the system for the truth on every
     * resume and push it into the board, so the Lock button and the edit lock
     * can never sit stuck in a state the OS has already abandoned.
     */
    @Override
    protected void onResume() {
        super.onResume();
        boolean pinned = isSystemLockTaskActive();
        boardLocked = pinned;
        if (web != null) {
            web.evaluateJavascript(
                    "window.setPinnedState && setPinnedState(" + pinned + ", false)", null);
        }
    }

    private boolean isSystemLockTaskActive() {
        try {
            ActivityManager am = (ActivityManager) getSystemService(ACTIVITY_SERVICE);
            return am != null
                    && am.getLockTaskModeState() != ActivityManager.LOCK_TASK_MODE_NONE;
        } catch (Exception e) {
            android.util.Log.w("AAC_APP", "lock task state query failed: " + e);
            return boardLocked;
        }
    }
