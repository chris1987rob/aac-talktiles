...
# --- assets: the board itself, plus the offline subject-segmentation runtime ---
cp "$ROOT/index.html" "$APP/assets/index.html"
# ONNX Runtime (WASM) + the segmentation model. Shipped inside the APK so the
# board stays fully offline -- the whole point of the app is that it works with
# no network at all.
rm -rf "$APP/assets/assets"
mkdir -p "$APP/assets/assets"
cp "$ROOT/assets/ort.min.js" "$APP/assets/assets/"
cp "$ROOT/assets/ort-wasm-simd.wasm" "$APP/assets/assets/"
cp "$ROOT/assets/subject.onnx" "$APP/assets/assets/"
